/**
 * SketchDrawController — four draw tools (rectangle, circle, polygon, lasso)
 * for SketchLayoutCanvas. Extracted from sketch-layout-canvas.js.
 *
 * Constructor args:
 *   drawLayer        SVGGElement  — stable, built once in _build()
 *   drawHandlesLayer SVGGElement  — screen-space overlay layer
 *   getViewport      () => { scale, panX, panY }
 *   callbacks        { onShapeCreated }
 */

import { svgEl, ringToPath, handleRectAttrs } from "./transform.js";
...
export class SketchDrawController {
  ...
  /** Close polygon on double-click; handles duplicate-vertex cleanup. */
  onDblClick() {
  ...
  /** Cancel any in-progress draw (Escape key, tool change). */
  cancel() {
    if (!this.#drawState) return;
    const ds = this.#drawState;
    this.#drawState     = null;
  ...
  #updateRectPreview(bx, bz) {
    ...
    const { min_x: minX, max_x: maxX, min_z: minZ, max_z: maxZ } =
      drawnBoundsFromBlocks(startBx, startBz, bx, bz);
  ...
  #completeRect() {
    ...
    this.refreshDrawHandles();
    const { min_x: minX, max_x: maxX, min_z: minZ, max_z: maxZ } =
      drawnBoundsFromBlocks(startBx, startBz, currentBx, currentBz);
